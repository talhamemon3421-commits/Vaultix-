...
const initiateUpload = async (userId, folderId, files) => {
  ...
  if (folderId !== null) {
    ...
    if (folder.user_id !== userId) {
      throw new Error('FOLDER_NOT_FOUND'); // resource hiding
    }
  }

  // check duplicate names within request
  const names = files.map(f => sanitizeFileName(f.originalName.trim()));
  ...
  const fileRecords = files.map(file => {
    ...
    const sanitizedName = sanitizeFileName(file.originalName.trim());
  ...
  let insertedFiles;
  try {
  insertedFiles = await insertFileBatchTransaction(fileRecords);
} catch (err) {
  if (err.code === '23505') {
    throw new Error('NAME_CONFLICT');
  }
